...
|---|---|---|---|---|
| **codex** | `^[[:space:]]*[^[:space:]]+( [a-z]+)? · ` (model-agnostic: one model token, an optional effort word, then the middot before the cwd — the model comes from the user's config; anchoring the whole line also rejects notices that contain `·`) | Bottom status line `<model> <effort> · /path`, e.g. `gpt-6-astra medium · /path` | "Hooks need review" on first run → send `2` Enter ("Trust all and continue"). Auth: `codex login` if "Not authenticated". | Non-interactive bash must use the tmux TUI (not bare `codex`); sandbox via `-s read-only` / `-s workspace-write`; reasoning effort via `-c model_reasoning_effort=<level>` only when the user asks (default: the codex config). File-include: `@path`. |
| **claude** (Claude Code CLI) | *(none — see spawn notes)* busy marker: `esc to interrupt` | Idle footer is USER-CONFIGURABLE (custom statuslines, vim mode), so no positive idle marker is reliable; while a turn runs the TUI shows "esc to interrupt" near the input box — use stability + busy-marker-absent. | Theme picker on very first launch; per-directory trust prompt ("Quick safety check") → `Down` then `Enter` (the cursor starts on "No, exit"; a bare Enter quits). Auth: `claude /login`. | Model via `--model` (empty = user's default); write mode via `--permission-mode acceptEdits`; resume with `claude --continue`. File-include: `@path`. |
